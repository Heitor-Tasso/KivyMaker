...
.. image:: images/ScaleWidget.gif
    :align: right

:class:`ScaleWidget` is used to build interactive widgets that can be translated,
rotated and scaled with two or more fingers on a multitouch system.

ScaleWidget has its own matrix transformation: the modelview matrix is changed
...
properties. That specific behavior makes the ScaleWidget unique, but there are some
advantages / constraints that you should consider:

#. The children are positioned relative to the ScaleWidget similarly to a
   :mod:`~kivy.uix.relativelayout.RelativeLayout`. So when dragging the
   ScaleWidget, the position of the children don't change, only the position of
   the ScaleWidget does.
#. The ScaleWidget size has no impact on the size of its children.
#. If you want to resize the ScaleWidget, use scale, not size (read #2). Scale
   transforms both the ScaleWidget and its children, but does not change size.
#. The ScaleWidget is not a layout. You must manage the size of the children
   yourself.

For touch events, the ScaleWidget converts from the parent matrix to the ScaleWidget
...
By default, the ScaleWidget does not have a graphical representation: it is a
container only. The idea is to combine the ScaleWidget with another widget, for
example an :class:`~kivy.uix.image.Image`::

    ScaleWidget = ScaleWidget()
    image = Image(source='sun.jpg')
    ScaleWidget.add_widget(image)
...
Disable rotation::
    ...
    ScaleWidget = ScaleWidget(do_rotation=False)

Allow only translation::

    ScaleWidget = ScaleWidget(do_rotation=False, do_scale=False)
...
If the :attr:`ScaleWidget.auto_bring_to_front` property is True, the ScaleWidget
...
when you are manipulating several ScaleWidget widgets and don't want the active
...
You can also limit the minimum and maximum scale allowed::
    ...
    ScaleWidget = ScaleWidget(scale_min=.5, scale_max=3.)
...
__all__ = ('ScaleWidget', 'ScaleWidgetPlane')
...
class ScaleWidget(Scatter):
    '''ScaleWidget class. See module documentation for more information.

    :Events:
        `on_transform_with_touch`:
            Fired when the ScaleWidget has been transformed by user touch
            or multitouch, such as panning or zooming.
        `on_bring_to_front`:
            Fired when the ScaleWidget is brought to the front.
    ...
    ScaleWidget will be done after dispaching the touch to the children.
    You can put children outside the bounding box of the ScaleWidget and still be
    able to touch them.
    ...
    .. note::
        ...
        please use the :attr:`~ScaleWidget.apply_transform` method.
    ...
    rotation = AliasProperty(_get_rotation, _set_rotation,
                             bind=('x', 'y', 'transform'))
    '''Rotation value of the ScaleWidget in degrees moving in a counterclockwise
    direction.
    ...
    '''Scale value of the ScaleWidget.
    ...
    def __init__(self, **kwargs):
        ...
        super(ScaleWidget, self).__init__(**kwargs)
    ...
    def _apply_transform(self, m, pos=None):
        ...
        return super(ScaleWidget, self)._apply_transform(m, (0, 0))

    def apply_transform(self, trans, post_multiply=False, anchor=(0, 0)):
        '''
        Transforms the ScaleWidget by applying the "trans" transformation
        matrix (on top of its current transformation state). The resultant
        matrix can be found in the :attr:`~ScaleWidget.transform` property.

        :Parameters:
            `trans`: :class:`~kivy.graphics.transformation.Matrix`.
                Transformation matix to be applied to the ScaleWidget widget.
        ...
        Usage example::
            ...
            ScaleWidget_instance.apply_transform(mat)
    ...
    def on_touch_down(self, touch):
        ...
        if super(ScaleWidget, self).on_touch_down(touch):
            touch.pop()
    ...
    def on_touch_move(self, touch):
        ...
        if self.collide_point(x, y) and not touch.grab_current == self:
            touch.push()
            touch.apply_transform_2d(self.to_local)
            if super(ScaleWidget, self).on_touch_move(touch):
                touch.pop()
                return True
            touch.pop()
    ...
    def on_transform_with_touch(self, touch):
        '''
        Called when a touch event has transformed the ScaleWidget widget.
    ...
    def on_bring_to_front(self, touch):
        '''
        Called when a touch event causes the ScaleWidget to be brought to the
        front of the parent (only if :attr:`auto_bring_to_front` is True)

        :Parameters:
            `touch`:
                The touch object which brought the ScaleWidget to front.
    ...
    def on_touch_up(self, touch):
        ...
        if not touch.grab_current == self:
            touch.push()
            touch.apply_transform_2d(self.to_local)
            if super(ScaleWidget, self).on_touch_up(touch):
                touch.pop()
                return True
            touch.pop()
...
class ScaleWidgetPlane(ScaleWidget):
    '''This is essentially an unbounded ScaleWidget widget. It's a convenience
    ...
    def __init__(self, **kwargs):
        ...
        super(ScaleWidgetPlane, self).__init__(**kwargs)
